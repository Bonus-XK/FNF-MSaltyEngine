...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		scoreTxt = new FlxText(-450, healthBarBG.y + 36, FlxG.width, "", 20);
		...
		scoreTxt1 = new FlxText(0, healthBarBG.y + 36, FlxG.width, "", 20);
		scoreTxt1.setFormat(Paths.font("vcr.ttf"), 17, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt1.scrollFactor.set();
		scoreTxt1.borderSize = 1.25;
		scoreTxt1.visible = !ClientPrefs.hideHud;
		add(scoreTxt1);
	...
	{
		if(ClientPrefs.changeLanguages == "Chinese")
		{
			scoreTxt.text = '分数：' + songScore + " | "
			+ ' 失误：' + songMisses + " | ";

			scoreTxt1.text = '评级：' + ratingName
			+ (ratingName != 'N/A' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 2)}%) - $ratingFC' : ''); // 聪明
		}
		else{
			scoreTxt.text = 'Score:' + songScore + " | "
			+ ' Misses: ' + songMisses + " | ";

			scoreTxt1.text = 'Rank: ' + ratingName
		...
		if(ClientPrefs.scoreZoom && !miss && !cpuControlled)
		{
			...
			scoreTxt1.scale.x = 1.075;
			scoreTxt1.scale.y = 1.075;
			...
			scoreTxtTween = FlxTween.tween(scoreTxt1.scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween) {
					scoreTxtTween = null;
				}
			});
		...
		if(health <= 0.35)
		{
			...
			scoreTxt1.setFormat(Paths.font("vcr.ttf"), 17, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}
		else if(health >= 1.65)
		{
			scoreTxt.setFormat(Paths.font("vcr.ttf"), 17, FlxColor.LIME, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			scoreTxt1.setFormat(Paths.font("vcr.ttf"), 17, FlxColor.LIME, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		}
		else
		{
			scoreTxt.setFormat(Paths.font("vcr.ttf"), 17, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			ratingFC = "N/A";
			if (ClientPrefs.changeLanguages == "Chinese"){
				if (sicks > 0) ratingFC = ":全连";
			    if (goods > 0) ratingFC = ".全连";
				if (bads > 0 || shits > 0) ratingFC = "全连";
				if (songMisses > 0 && songMisses < 10) ratingFC = ".有失误";
				else if (songMisses >= 10) ratingFC = ":有失误";
			}else{
